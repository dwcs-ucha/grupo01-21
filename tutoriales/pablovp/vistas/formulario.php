...
<head>
    ...
    $mensaxe = "";
        if(isset($_POST['submit'])) { //si el usuario envía el formulario
            /** Validación Ej. 1 **/
                if(isset($_POST['misBloques'])) { //si tiene cubierta la primera pregunta
                    if($_POST['misBloques']==$preguntas[0]->respuesta) { //y es igual a la respuesta del objeto en su posición
...
<body>
    ...
    <div class="container p-4">
        <div class="row">
            <div class="col-md-12">
                <h1 class="my-4 text-center text-primary">Examen del tutorial de Flappy Bird</h1>
            </div>
        </div>
    </div>
    ...
    <form action="./formulario.php" method="post">
        <p><?php echo $preguntas[0]->cod;?>. <?php echo $preguntas[0]->enunciado;?></p>
            <select name="misBloques">
                <option value="" <?php !isset($_POST['misBloques']) ? print 'selected="true"': ""?> disabled>Selecciona</option>
                <option value="<?php print $preguntas[0]->respuesta; ?>">Mis bloques</option>
